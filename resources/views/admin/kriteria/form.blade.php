<div class="modal-body">
    ...
    <div>
        ...
        <input class="form-control" type="number" step="0.01" min="0" max="1" name="nilai_bobot" value="{{ old('nilai_bobot', $row->nilai_bobot ?? '') }}" required>
        @php
            $currentVal = isset($row) ? (float) $row->nilai_bobot : 0;
            $maxForThis = 1.0 - (($totalWeight ?? 0) - $currentVal);
        @endphp
        <div class="form-text text-muted">Batas bobot: {{ number_format(max(0, $maxForThis), 2) }}</div>
    </div>
</div>
